        ...
        view! {
            <div>
                <div class="raptor-grid">
                    ...
                    <Metric label="Primary lane" value=move || contrib.get().map(|s| format!("{} ok / {} failed", s.runtime.relay_session.primary_lane_objects_succeeded, s.runtime.relay_session.primary_lane_objects_failed)).unwrap_or_else(|| "—".to_owned()) detail="per-object carrier outcome" />
                    <Metric label="Warm lane" value=move || contrib.get().map(|s| format!("{} ok / {} failed", s.runtime.relay_session.secondary_lane_objects_succeeded, s.runtime.relay_session.secondary_lane_objects_failed)).unwrap_or_else(|| "—".to_owned()) detail="per-object carrier outcome" />
